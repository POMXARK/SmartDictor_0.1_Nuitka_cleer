...
log  --show-progress --follow-import-to=choice_of_voice_engine --follow-import-to=silero_speaker --follow-import-to=Say --windows-disable-console --follow-import-to=pitch_voise  main.py
...
v2 --follow-import-to=cycler --follow-import-to=matplotlib --follow-import-to=numpy --follow-import-to=scipy --follow-import-to=PIL  --follow-import-to=setuptools  main.py
...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        """Кнопки и визуал графического интерфейса"""
        ...
        self.comboBox.activated[str].connect(self.onActivated)  # вызвать функцию при изменении текста в combo box
        ...
        index = self.comboBox_2.findText(voise_name, QtCore.Qt.MatchFixedString)  # выбрать нужную строку в списке
        if index >= 0:
            self.comboBox_2.setCurrentIndex(index)
        self.comboBox_2.activated[str].connect(self.VoiceActivated)  # вызвать функцию при изменении текста в combo box
    ...
    # print(text)
    # self.pushButton.setText(text) # выбирает нужный текст
...
class thread(QThread):  # создание и функции потока
    ...
    def run(self):  # запуск потока
        ...
        while True:  # бесконечный цикл
            if e.edit_img():  # Проверка нового кадра
