...
def detect_line():
    ...
    img = cv2.inRange(img, (int(set_opt()['mb_h1']), int(set_opt()['mb_s1']), int(set_opt()['mb_v1'])), (
        int(set_opt()['mb_h2']), int(set_opt()['mb_s2']),
        int(set_opt()['mb_v2'])))  # (0, 160, 100), (255, 255, 255) # выделение линий
    cv2.imwrite("mask.png", img)
    mask_img = cv2.imread('mask.png')
    show('mask', mask_img)
    binary = cv2.bitwise_not(img)

    img = cv2.Canny(img, 50, 50, apertureSize=3)  # инвертирует изображение выделяя контуры, Обнаружение хитрого края
