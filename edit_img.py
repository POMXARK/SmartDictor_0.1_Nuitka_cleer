...
from For_tesseract import tesseract_mask
...
def edit_img():
    ...
    img = screenshot(
        region=(int(set_opt()['X']), int(set_opt()['Y']), Width - int(set_opt()['отступ с краев экрана по ширине']),
    ...
    if difference_images():  # дубликат предыдущей сцены - пропуск
        # print('ЛИШНИЙ КАДР')
        return False
    else:
        print('else')
        ...
        tesseract_mask()  # обработка для понимая tesseract
        # sil.say_silero()
